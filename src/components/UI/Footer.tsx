...
export interface FooterLink {
    url: string;
    label: string;
    /** Raw SVG markup string (`?raw` import in config) — injected verbatim. */
    icon: string;
}
...
export default function Footer({ config }: FooterProps) {
    ...
    return (
        <footer className="relative bottom-0 w-full h-[50vh] flex justify-between items-center flex-col px-4 py-8 bg-footer">
            <svg
                id="visual"
                version="1.1"
                viewBox="0 321.18 960 219.82"
                className="absolute -translate-y-full pb-4 w-full z-[-1]"
            ...
            <div className="w-full flex_center flex-col gap-2">
                <div className="flex_center w-[80px] h-[80px]">
